...
- Add a bounded manual source-maintenance collector, durable review queue, fail-closed persistence and recovery behavior, stable issue-evidence deduplication, and injected-source tests. Inaccessible sources and unresolved findings remain explicit; successful collection does not renew record dates or legal status.
- Pin the accessibility scan action by commit with a fail-closed major policy and retain the dated 160-route audit evidence.
...
- Correct shared matrix, timeline, breadcrumb, and navigation rendering so the 160-route major accessibility gate completes with no confirmed violations, incomplete candidates, or route errors; cover explicit light and dark theme behavior for the affected patterns.
...
## Scope verification included by v0.2.2

This scope-verification work was already present at the v0.2.2 tag and remains part of that release history.
