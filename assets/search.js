...
(function () {
  ...
  function highlightMatch(text, query) {
    ...
    for (var t = 0; t < tokens.length; t++) {
      var re = new RegExp('(' + tokens[t].replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')', 'gi');
  ...
  function renderResults(matches, query) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      html += '<span class="search-result-name">' + highlightMatch(m.name, query) + '</span>';
      var meta = m.type;
      if (m.type === 'jurisdiction') {
        meta += ' · ' + escapeHtml(m.status || 'unevaluated');
        if (m.instruments) meta += ' · ' + m.instruments + ' instrument' + (m.instruments !== 1 ? 's' : '');
      } else {
        if (m.jurisdiction) meta += ' · ' + escapeHtml(m.jurisdiction);
        if (m.group) meta += ' · ' + escapeHtml(m.group);
      }
      html += '<span class="search-result-type">' + meta + '</span>';
      html += '</li>';
    }
